...
extends Quad:
  ...
  def insert(b: Body): Quad = {
    val newBodies = bodies.appended(b)
    if (size <= minimumSize) {
      ...
      newBodies.foldLeft(fork)(_.insert(_))
    }
  }
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix = {
    val i = ((b.x - boundaries.minX) / (boundaries.width / sectorPrecision)).toInt
    val j = ((b.y - boundaries.minY) / (boundaries.height / sectorPrecision)).toInt
    apply(i, j) += b
    this
  }
  ...
  def combine(that: SectorMatrix): SectorMatrix = {
    this.matrix.zip(that.matrix).zipWithIndex.foreach((pair, i) => {
      this.matrix(i) = pair._1.combine(pair._2)
    })
    this
  }
