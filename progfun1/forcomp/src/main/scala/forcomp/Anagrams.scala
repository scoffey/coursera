...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.groupBy(identity).view.mapValues(_.length).toList.sorted
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def combinationsHelper(pair: (Char, Int), accum: List[Occurrences]): List[Occurrences] = accum ++ (for {
      tail <- accum
      i <- 1 to pair._2
    } yield (pair._1, i) :: tail)
    occurrences.foldRight(List[Occurrences](Nil))(combinationsHelper)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    (for ((char, count) <- x) yield (char, count - yMap(char))).filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ :: _ => for {
        ...
        head <- dictionaryByOccurrences.withDefaultValue(Nil)(combination)
        tail <- sentenceAnagramsHelper(subtract(occurrences, combination))
      } yield head :: tail
    }
