...
trait Huffman extends HuffmanInterface {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(subtree: CodeTree, bits: List[Bit]): List[Char] = subtree match {
      ...
      case Fork(left, right, _, _) =>
        traverse(if (bits.head == 0) left else right, bits.tail)
    }
    traverse(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def lookup(tree: CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(_, _) => List()
      case Fork(left, right, _, _) =>
        if (chars(left).contains(c)) 0 :: lookup(left)(c)
        else 1 :: lookup(right)(c)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.find(_._1 == char).getOrElse(
      throw new NoSuchElementException("Cannot find " + char + " in table " + table)
    )._2
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertHelper(tree: CodeTree, accumBits: List[Bit]): CodeTable = tree match {
      case Leaf(char, _) => List((char, accumBits))
      case Fork(left, right, _, _) => mergeCodeTables(
        convertHelper(left, accumBits.appended(0)),
        convertHelper(right, accumBits.appended(1))
      )
    }
