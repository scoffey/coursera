...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val leisurePrefixesExclusions = List("t1801", "t1803", "t1805")
    (
      columnNames.filter(c => primaryNeedsPrefixes.exists(c.startsWith)).map(new Column(_)),
      columnNames.filter(c => workingPrefixes.exists(c.startsWith)).map(new Column(_)),
      columnNames.filter(c => leisurePrefixes.exists(c.startsWith) && !leisurePrefixesExclusions.exists(c.startsWith)).map(new Column(_))
    )
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(x: Double): Double = {
      BigDecimal(x).setScale(1, BigDecimal.RoundingMode.HALF_UP).doubleValue()
    }
