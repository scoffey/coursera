...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setAppName("bigdata-wikipedia").setMaster("local")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def accumulator(accum: Int, article: WikipediaArticle): Int = {
      if (article.mentionsLanguage(lang)) accum + 1 else accum
    }
    rdd.aggregate(0)(accumulator, _ + _)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article => langs.filter(article.mentionsLanguage).map((_, article))).groupByKey()
  }
