...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(n) if n.isValidInt => n.toIntExact }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(items) => {
        val optionals = items.map(implicitly[Decoder[A]].decode)
        Option.when(optionals.forall(_.isDefined))(optionals.map(_.get))
      }
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fields) if fields.contains(name) => implicitly[Decoder[A]].decode(fields(name))
      case _ => None
    }
