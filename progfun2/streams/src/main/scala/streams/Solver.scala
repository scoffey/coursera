...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.to(LazyList).map((neighbor, move) => (neighbor, move :: history))
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filterNot((neighbor, move) => explored.contains(neighbor))
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, moves) #:: tail =>
      val neighbors = newNeighborsOnly(neighborsWithHistory(block, moves), explored)
      (block, moves) #:: from(tail ++ neighbors, explored + block)
  }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    pathsFromStart.filter((neighbor, move) => done(neighbor))
  }
